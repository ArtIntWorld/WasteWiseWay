...
//checking if request cotains multipart data
            ...
            if (isMultipart) {
                ...
                while (itr.hasNext())//checking if there is any value in the position like rs.next()
                {
                    ...
                    } else//when it is an image or multipart
                    {
                        ...
                        if (field_name.equals("file_logo"))//write the name of image here it is img_product
                        {
                            ...
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\AgencyLogo\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated

                            logo = "ALogo_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                ...
                String selQry = "select * from tbl_agency where district_id='" + value[4] + "'";
                ResultSet rs = obj.selectCommand(selQry);
                if (rs.next()) {
        %> 
        <script type="text/javascript">
            alert("Agency Alredy Exist in This place");
            setTimeout(function() {
                window.location.href = '../../Guest/Register.jsp'
            }, 40);
        </script>
        <%
        } else {
