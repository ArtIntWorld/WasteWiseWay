<%-- 
    Document   : SupervisorUploadAction
    Created on : 31 Dec, 2022, 10:02:13 AM
    Author     : ardra
--%>
...
//checking if request cotains multipart data
            ...
            if (isMultipart) {
                ...
                while (itr.hasNext())//checking if there is any value in the position like rs.next()
                {
                    ...
                    } else//when it is an image or multipart
                    {
                        ...
                        if (field_name.equals("file_photo"))//write the name of image here it is img_product
                        {
                            ...
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\SupervisorPhoto\\";//path for storing image
                        ...
                        if (field_name.equals("file_proof"))//write the name of image here it is img_product
                        {
                            ...
                            //setting path to store image
                            File proj_path = new File(config.getServletContext().getRealPath("/"));
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\SupervisorProof\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated
                ...
                String InsQry = "insert into tbl_supervisor(supervisor_name,supervisor_email,supervisor_address,supervisor_contact,supervisor_gender,place_id,supervisor_photo,supervisor_proof,supervisor_password,supervisor_doj,agency_id)"
                        + "values('" + value[0] + "','" + value[1] + "','" + value[2] + "','" + value[3] + "','" + value[4] + "','" + value[6] + "','" + photo + "','" + proof + "','" + value[7] + "',curdate(),'"+session.getAttribute("agid")+"')";
        ...
        <script type="text/javascript">
            ...
            setTimeout(function() {
              window.location.href = '../../Agency/AddSupervisor.jsp'
            }, 40);
        </script>
        <%
                }
